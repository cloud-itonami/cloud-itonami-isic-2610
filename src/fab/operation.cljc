(ns fab.operation
  "OperationActor -- one fab operation = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor (Fab Advisor)
  is sealed into a single node (:advise); its proposal is ALWAYS
  routed through the Fab Operations Governor (:govern) and the
  ...
  One graph run = one fab operation (intake -> advise -> govern ->
  ...
  decision to a human operator (the fab engineer). The approver
  resumes with `{:approval {:status :approved}}` (or :rejected).
  `:actuation/dispatch-process-step`/`:actuation/finalize-yield-audit`
  ALWAYS reach this node when the governor is clean -- see `fab.
  phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [fab.fabadvisor :as fabadvisor]
...
(defn build
  "Compiles an OperationActor graph bound to `store` (any `fab.store/
  Store`).
  opts:
    :advisor      -- a `fab.fabadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (fabadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Fab Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (fabadvisor/-advise advisor store request)]
            {:proposal p :audit [(fabadvisor/trace request p)]})))

      ;; Fab Operations Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
